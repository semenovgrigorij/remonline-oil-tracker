...
async function getOrders(dateFilter) {
  try {
    ...
    while (hasMorePages) {
      ...
      console.log(`✅ Страница ${page}: получено ${data.data.length} заказов (всего: ${allOrders.length})`);
      console.log(`📊 Общая информация: страница ${page} из ${data.total_pages}, всего заказов: ${data.count}`);
    ...
    console.log(`🎯 Завершено! Получено ${allOrders.length} заказов со всех ${page - 1} страниц`);
    return allOrders;
  } catch (error) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (response.status === 429) {
        ...
        console.log(`Rate limit hit for order ${orderId}, waiting ${waitTime}ms (attempt ${attempt}/${retries})`);
      ...
      if (attempt === retries) {
        console.error(`Final error fetching items for order ${orderId}:`, error.message);
        return [];
      }
      
      // Ждем перед повторной попыткой
      const waitTime = 1000 * attempt;
      console.log(`Error fetching order ${orderId}, retrying in ${waitTime}ms (attempt ${attempt}/${retries})`);
      await new Promise(resolve => setTimeout(resolve, waitTime));
    }
  }
...
async function analyzeOilUsage(orders, socketId = null) {
  ...
  for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
        ...
        if (orderOilQuantity > 0) {
          ...
          Object.keys(orderOilsByType).forEach(oilId => {
            if (orderOilsByType[oilId] > 0) {
              results.oilBreakdown[oilId].totalQuantity += orderOilsByType[oilId];
    ...
    if (socketId) {
      const progress = Math.round((results.processedOrders / results.totalOrders) * 100);
  ...
  try {
    ...
    while (hasMorePages) {
      ...
      const params = new URLSearchParams({
        branches: BRANCH_ID,
        page: page.toString(),
        page_size: '100'  // Увеличиваем до максимума для меньшего количества запросов
      });
      ...
      console.log(`📄 Запрашиваем страницу ${page}${totalPages > 1 ? ` из ${totalPages}` : ''}...`);
      
      // Отправляем статус через WebSocket
      if (socketId) {
        const progressPercent = totalPages > 1 ? Math.round((page / totalPages) * 100) : 0;
      ...
      if (page === 1) {
        ...
        console.log(`📊 Найдено ${totalCount} заказов на ${totalPages} страницах (по ${data.page_size} на страницу)`);
      }
      
      // Добавляем заказы с текущей страницы к общему списку
      allOrders = allOrders.concat(data.data || []);
      
      console.log(`✅ Страница ${page}: получено ${data.data ? data.data.length : 0} заказов (всего: ${allOrders.length})`);
...
app.get('/api/oil-usage', async (req, res) => {
  try {
    ...
    if (month) {
      ...
      console.log(`Fetching orders for ${periodLabel}: ${startDate} - ${endDate}`);
    } else {
    ...
    console.log(`✅ Получено ${orders.length} заказов для периода: ${periodLabel}`);
...
server.listen(PORT, () => {
  ...
  console.log(`🛢️ Tracking oils:`, Object.keys(OIL_PRODUCTS).map(id => `${OIL_PRODUCTS[id].name} (${id})`));
